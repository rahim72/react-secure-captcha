...
import { useState as useState2, useEffect as useEffect6, useCallback as useCallback3 } from "react";
...
import { jsx as jsx5, Fragment } from "react/jsx-runtime";
...
function PopChild({ children, isPresent, anchorX, root }) {
  ...
  return jsx3(PopChildMeasure, { isPresent, childRef: ref, sizeRef: size, children: React2.cloneElement(children, { ref }) });
}
...
var PresenceChild = ({ children, initial, isPresent, onExitComplete, custom, presenceAffectsLayout, mode, anchorX, root }) => {
  ...
  if (mode === "popLayout") {
    children = jsx4(PopChild, { isPresent, anchorX, root, children });
  }
...
import { useContext as useContext2, useId as useId3, useEffect as useEffect4, useCallback } from "react";
...
var AnimatePresence = ({ children, custom, initial = true, onExitComplete, presenceAffectsLayout = true, mode = "sync", propagate = false, anchorX = "left", root }) => {
  ...
  return jsx5(Fragment, { children: renderedChildren.map((child) => {
    const key = getChildKey(child);
    ...
    return jsx5(PresenceChild, { isPresent, initial: !isInitialRender.current || initial ? void 0 : false, custom, presenceAffectsLayout, mode, root, onExitComplete: isPresent ? void 0 : onExit, anchorX, children: child }, key);
  }) });
};
...
import { useContext as useContext6, useRef as useRef5, useInsertionEffect as useInsertionEffect2, useEffect as useEffect5 } from "react";
...
function useVisualElement(Component3, visualState, props, createVisualElement, ProjectionNodeConstructor) {
  ...
  const wantsHandoff = useRef5(Boolean(optimisedAppearId) && !window.MotionHandoffIsComplete?.(optimisedAppearId) && window.MotionHasOptimisedAnimation?.(optimisedAppearId));
  useIsomorphicLayoutEffect(() => {
    if (!visualElement)
      return;
...
function createMotionComponent(Component3, { forwardMotionProps = false } = {}, preloadedFeatures, createVisualElement) {
  ...
  function MotionDOMComponent(props, externalRef) {
    ...
    return jsxs(MotionContext.Provider, { value: context, children: [MeasureLayout2 && context.visualElement ? jsx6(MeasureLayout2, { visualElement: context.visualElement, ...configAndProps }) : null, useRender(Component3, props, useMotionRef(visualState, context.visualElement, externalRef), visualState, isStatic, forwardMotionProps)] });
  }
...
function MeasureLayout(props) {
  ...
  return jsx7(MeasureLayoutWithContext, { ...props, layoutGroup, switchLayoutGroup: useContext8(SwitchLayoutGroupContext), isPresent, safeToRemove });
}
...
var Captcha = ({
  ...
  const refreshCaptcha = useCallback3(() => {
    if (type === "canvas") {
  ...
  const validateCaptcha = useCallback3(
    (value) => {
      const valid = type === "canvas" && value === captchaText || type === "math" && value === mathAnswer;
      if (valid) {
  ...
  const handleInputChange = (e) => {
    ...
    setInputValue(val);
    if (autoValidate && val.length >= (type === "canvas" ? length : mathAnswer.length)) {
      validateCaptcha(val);
    }
  };
...
function useCaptcha(initialValue = "") {
  const [captcha, setCaptcha] = useState3(initialValue);
  const refresh = useCallback4((newValue) => {
    setCaptcha(newValue);
  }, []);
...
export {
  CanvasCaptcha_default as CanvasCaptcha,
  Captcha_default as Captcha,
  MathCaptcha_default as MathCaptcha,
  useCaptcha
};
